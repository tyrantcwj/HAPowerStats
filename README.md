# HAPowerStats - 家庭用电监控系统

对接 Home Assistant 的家庭用电统计程序：**只填 HA 地址和令牌，插座设备直接勾选导入，支持多个插座**。
功率、电量、电压、电流等参数会从 HA 实体自动绑定，无需手动填实体 ID。

## 功能特性

- **只配连接**：HA 只需要地址 + 长期访问令牌
- **一键导入插座**：自动扫描 HA 里带功率 / 电量计量的设备，勾选即导入
- **参数自动识别**：功率(W/kW)、电量(kWh)、电压、电流、功率因数、频率、开关实体自动绑定并统一单位
- **小米插座打包数据解码**：自动拆解米家 MIoT 把多项数据挤在一个大整数里的寄存器，
  让功率/电压/今日/本月电量和米家 App 对得上
- **多插座支持**：每个插座独立统计，另有全屋合计；实时曲线一线一插座
- 今日 / 本月用电自动统计，每日 0 点、每月 1 号自动重置
- 有电量实体的直接用 HA 累计值（自动处理实体归零），没有的按功率积分推算
- 插座开关控制（直接调用 HA 服务，不依赖 MQTT）
- **实时天气与气温**：优先读 HA 的天气实体，没有则用 Open-Meteo（免密钥，默认上海浦东）
- **天气随数据一起记录**：每条采样都带当时的气温和天气，方便分析用电与天气的关系
- **使用时段报表**：自动识别电器每次「启动→停止」，一次一行，附时长、用电量和当时的气温天气
- 历史查询、按插座筛选
- 报表按时间段导出（含「最近 7 天」一键按钮），中文表头 + BOM，Excel 打开不乱码
- 可选：MQTT 自动发现把统计结果回写成 HA 实体；MySQL 长期存储
- 累计电能持久化，程序重启不清零

## 快速开始

### 方式一：Docker Compose 部署（推荐）

```bash
git clone https://github.com/tyrantcwj/HAPowerStats.git
cd HAPowerStats
docker compose up -d --build
```

也可以直接使用已发布镜像：

```yaml
services:
  electricity-monitor:
    image: ghcr.io/tyrantcwj/hapowerstats:latest
    container_name: electricity-monitor
    restart: unless-stopped
    ports:
      - "5000:5000"
    volumes:
      - ./data/electricity_data:/app/electricity_data
      - ./data/config:/app/config
    environment:
      - TZ=Asia/Shanghai
```

访问：`http://你的服务器IP:5000`（无需登录，建议只在内网使用）

### 方式二：本地运行

```bash
pip install -r requirements.txt
python home_electricity_monitor.py
```

## 配置流程

打开 Web 页面 → **⚙️ 系统设置**，三步即可：

1. **填 HA 连接**：HA 地址（如 `http://192.168.1.100:8123`）+ 长期访问令牌
   （HA 左下角点击用户名 → 长期访问令牌 → 创建令牌），点「保存」
2. **扫描插座**：点「🔍 扫描设备」，程序会列出所有带功率或电量计量的设备，
   每个设备下面直接显示识别到的参数和当前读数
3. **勾选导入**：勾选要监控的插座（可多选），点「➕ 导入选中」

导入后采集程序会在下一个采集周期（默认 10 秒）自动生效，**不需要重启**。

### 自动识别的参数

| 角色 | 识别依据 | 统一单位 |
|------|----------|----------|
| 功率 | `device_class=power` 或单位 W/kW | W |
| 电量 | `device_class=energy` 或单位 Wh/kWh/MWh | kWh |
| 电压 | `device_class=voltage` | V |
| 电流 | `device_class=current` | A |
| 功率因数 / 频率 | 对应 `device_class` | - |
| 开关 | 同设备下的 `switch.*` 实体 | - |

说明：

- 归组优先用 HA 的**设备**信息（通过模板接口读取 `device_id`），拿不到时按实体名前缀归组
- 同一设备有多个电量实体时，优先取累计总量（`total_increasing`），自动跳过「今日 / 本月用电」这类分段统计
- 既没有功率也没有电量的设备不会出现在列表里（温度传感器等不会被误识别）

### 小米（米家）插座的打包数据

米家 MIoT 协议会把多项数据打包进一个 32 位整数，HA 的米家集成没有拆包，于是在 HA 里
只能看到「电费提醒3 = 14418164」这种没有单位的大整数，和米家 App 显示的数字对不上。
本程序会自动识别并拆包（`mi_decode.py`）：

| HA 里的实体 | 拆出来的数据 | 位分割 |
|-------------|--------------|--------|
| `电计量设置 电费提醒3` | 今日电量、本月电量（kWh） | 今日 = `值 >> 18` / 100，本月 = `值 & 0xFFFF` / 100 |
| `电计量设置 功率, 电压` | 实时功率（W）、电压（V） | 功率 = `值 >> 12` / 100，电压 = `值 & 0xFFF` / 10 |

两台真机对照米家 App 的验证结果：

| 原始值 | 拆包结果 | 米家 App |
|--------|----------|----------|
| `14418164` | 今日 0.55 / 本月 2.44 kWh | 0.55 / 2.44 ✅ |
| `52166863` | 今日 1.99 / 本月 2.07 kWh | 1.99 / 2.07 ✅ |
| `41101530` | 100.34 W / 226.6 V | 100.78 W / 226.40 V（实时波动）|
| `579810` | 1.41 W / 227.4 V | 1.54 W / 225.50 V（实时波动）|

行为说明：

- 只对**名字**匹配（电费提醒 / 功率,电压）且解出来数值合理的寄存器生效，
  「设置参数1」这类配置寄存器不会被误当成数据
- 真实实体优先，只有该角色没有正规实体时才用解包值
- 插座自己报了今日/本月电量时，页面直接用它（和米家一致），不再用本程序自己的日/月基线，
  设备卡片上会标「今日/本月取自插座」
- **升级前就导入过的插座**：解码配置是在「扫描 → 导入」时写进 `devices.json` 的，
  老配置里没有。重新扫描时这类插座会标黄色「可更新：功率、电压…」并自动勾选，
  点一次「导入选中」即可补上，名字、启用状态、电量统计方式都不会被覆盖

### 电量与电流是怎么来的

| 数据 | 来源 |
|------|------|
| 累计电量 | 优先用插座自身的累计计数器（卡片标「累计 (插座)」）；没有电量实体的插座才由本程序按功率积分累加 |
| 今日 / 本月 | 插座自报就直接用（和米家一致）；否则用累计值减去当日/当月起始值 |
| 电流 | 有电流实体就用实测值；没有的话按 `功率 ÷ 电压` 推算，卡片标「电流 (推算)」——米家 App 显示的电流也是这么来的 |
| 报表里的用电量 | 用功率对时间积分，不用累计计数器相减（小米插座的计数器是整度跳变，差值会一格一格跳）|

插座的电量实体被清零 / 换设备时，程序会把清零前的读数记进偏移量，累计值不会倒退。

### 天气与气温

「⚙️ 系统设置 → 🌤️ 天气与气温」：

| 数据来源 | 说明 |
|----------|------|
| 自动（默认） | 优先用 HA 里的 `weather.*` 实体（不额外请求外网），取不到再用 Open-Meteo |
| 只用 HA 天气实体 | 指定某个 weather 实体，比如和风天气、彩云、met.no |
| 只用 Open-Meteo | 免密钥公共接口，按经纬度取，默认上海浦东 (31.2231, 121.5397) |
| 不记录天气 | 关掉 |

天气会**跟着每次采样一起写进历史记录**，所以导出报表时能标出「这段时间用了多少电、
当时多少度、什么天气」。Open-Meteo 每 10 分钟取一次，不会频繁打外网。

### 导出报表

历史数据页底部的「📥 导出报表」，**按时间段导出**，右上角有「最近 7 天 / 最近 30 天 / 今天」快捷按钮：

| 报表 | 内容 |
|------|------|
| **使用时段** | 电器每次启动到停止一行：日期、开始/结束时间、时长、用电量、平均/最大功率、**平均气温、天气**；底部附每日汇总（当天启动几次、共多少度）|
| 每日统计 | 每天一行：用电量、平均/最大功率、采样点数、平均气温、天气 |
| 分时统计 | 每小时一段，同上 |
| 用电明细 | 原始采样点，可按每分钟 / 5 分钟 / 15 分钟抽样，含电压、电流、气温、天气 |

都可以只导某一个插座；页面上填了电价的话会多一列「电费(元)」。
文件名是中文的（如 `使用时段_2026-08-13_至_2026-08-19.csv`），带 BOM，Excel 双击打开不乱码。

#### 使用时段是怎么判定的

- 功率超过「启动判定功率」就算电器在工作，默认 5 W（可在每个插座上单独设，导出时也能临时改）
- 中途掉下去但在「合并间隔」（默认 10 分钟）内又起来，算**同一次**使用
  —— 洗衣机漂洗间歇、空调压缩机停机都属于这种
- 短于「最短时长」（默认 2 分钟）的忽略，滤掉功率抖动

举例：洗衣机某天 3:00-4:00、7:00-10:00、11:00-12:00 跑了三次，报表就是三行，
每行带这段时间的实际用电量和当时的气温天气。

### 已添加插座的管理

「🔌 已添加的插座」面板里可以改名、停用、移除，也能选择电量统计方式：

| 方式 | 说明 |
|------|------|
| 自动（默认） | 有电量实体就用电量实体，没有就按功率积分 |
| 用电量实体 | 强制读 HA 累计电量实体 |
| 功率积分 | 忽略电量实体，用功率对时间积分 |

### 可选配置

| 配置项 | 说明 |
|--------|------|
| MQTT 服务器 | 配置后把「合计 + 每个插座」的功率/总电能/今日/本月回写成 HA 实体 |
| MySQL | 长期存储，表结构带设备维度；不配则只写本地 CSV |
| `WEB_PORT` / `WEB_HOST` | 环境变量，改 Web 服务监听地址和端口（默认 `0.0.0.0:5000`） |

## 从旧版本升级

旧版本只能配一个 `power_entity_id`，升级后会自动迁移：

- 原来的功率实体变成一个名为「默认功率实体」的插座（key 为 `legacy_main`）
- 历史累计电量、今日 / 本月起始值原样继承，不会清零
- 旧的 `electricity_YYYY-MM.csv` 仍可查询，归到该设备名下；新数据写入 `records_YYYY-MM.csv`（带设备列）
- MySQL 老表会自动补上 `device_key` / `device_name` / `voltage_v` / `current_a` 列

## 文件结构

```
├── home_electricity_monitor.py   # 主程序：多设备采集、电量统计、MQTT 上报
├── ha_client.py                  # HA REST 客户端 + 插座设备自动发现
├── mi_decode.py                  # 小米插座打包寄存器解码
├── config_store.py               # 配置读写（HA 连接、插座列表、MQTT、MySQL）
├── storage.py                    # 历史存储：CSV / MySQL（按设备维度）
├── reports.py                    # 导出报表：使用时段 / 分时 / 每日 / 明细
├── weather.py                    # 天气与气温采集（HA 实体 / Open-Meteo）
├── web_server.py                 # Web 服务与 API
├── templates/
│   └── index.html                # 监控 / 历史 / 设置页面
├── requirements.txt
├── Dockerfile
├── docker-compose.yml
└── .github/workflows/            # 自动构建镜像、同步 Gitee
```

## 数据存储

| 位置 | 内容 |
|------|------|
| `config/ha_config.json` | HA 地址与令牌 |
| `config/devices.json` | 已导入的插座及其实体映射、启动判定功率 |
| `config/weather_config.json` | 天气来源与坐标 |
| `electricity_data/electricity_state.json` | 每个插座的累计电量等运行状态 |
| `electricity_data/records_YYYY-MM.csv` | 明细记录（时间、设备、功率、累计电量、电压、电流） |

## 主要 API

| 接口 | 说明 |
|------|------|
| `GET /api/realtime` | 合计 + 每个插座的实时数据 |
| `GET /api/ha/discover` | 扫描 HA 中的可用插座设备 |
| `POST /api/devices/import` | 按 key 批量导入插座 |
| `GET /api/devices` / `POST /api/devices` | 读取 / 保存插座配置 |
| `DELETE /api/devices/<key>` | 移除插座 |
| `GET /api/history?date=&device=` | 历史查询，`device` 为空表示全部合计 |
| `GET /api/export?type=&start=&end=&device=&interval=&price=&standby=` | 导出报表，`type` 为 `sessions`/`daily`/`hourly`/`detail` |
| `GET /api/weather` | 当前天气 + 可选的 HA 天气实体列表 |
| `POST /api/switch` | 开关插座（`device_key` + `action=on/off`） |

## License

MIT License
